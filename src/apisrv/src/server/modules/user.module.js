      ...
      if (connectionError) {
        ...
        connection.query('INSERT INTO user SET ?', insertValues, (error, result) => { // User資料表寫入一筆資料
          if (error) {
            ...
            resolve(`新增成功！ userID: ${result.insertId}`); // 寫入成功回傳寫入id
          }
          connection.release();
        });
      }
  ...
  return new Promise((resolve, reject) => {
    connectionPool.getConnection((connectionError, connection) => { // 資料庫連線
      if (connectionError) {
        ...
        connection.query('UPDATE user SET ? WHERE userID = ?', [insertValues, userId], (error, result) => {
          if (error) {
  ...
  return new Promise((resolve, reject) => {
    connectionPool.getConnection((connectionError, connection) => { // 資料庫連線
      if (connectionError) {
        ...
        connection.query('DELETE FROM user WHERE userID = ?', userId, (error, result) => {
          if (error) {
